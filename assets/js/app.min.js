...
//= ================
// LettersAnimation
const title = document.querySelectorAll('._letter-animation');
if (title) {
  for (let index = 0; index < title.length; index++) {
    const el = title[index];
    const txt = el.innerHTML;
    const txt_words = txt.replace('  ', ' ').split(' ');
    let new_title = '';
    for (let index = 0; index < txt_words.length; index++) {
      const txt_word = txt_words[index];
      const len = txt_word.length;
      new_title += '<p>';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function checkAndClosePopup() {
    ...
    if (popup) {
      ...
      setTimeout(() => {
        location.reload();
      }, 3000); // Перезагрузка страницы через 3 секунды после закрытия
    }
  }
...
// ShowMore Beta ========================
const moreBlocks = document.querySelectorAll('._more-block');
if (moreBlocks.length > 0) {
  const wrapper = document.querySelector('.wrapper');
  for (let index = 0; index < moreBlocks.length; index++) {
    const moreBlock = moreBlocks[index];
    const items = moreBlock.querySelectorAll('._more-item');
    if (items.length > 0) {
      const itemsMore = moreBlock.querySelector('._more-link');
      const itemsContent = moreBlock.querySelector('._more-content');
      const itemsView = itemsContent.getAttribute('data-view');
      if (getComputedStyle(itemsContent).getPropertyValue('transition-duration') === '0s') {
        itemsContent.style.cssText = 'transition-duration: 1ms';
      }
      itemsMore.addEventListener('click', (e) => {
      ...
      function updateSize() {
        const isScrollEnd = window.innerWidth - wrapper.offsetWidth;
...
// =================================
// Scrollbar на Главной ст.
const porgressBar = document.querySelector('.progress__line');
const progressContent = document.querySelector('.page');
if (porgressBar) {
  document.onscroll = function () {
    const porgressLine = (((window.scrollY + 260) / (progressContent.clientHeight - (window.innerHeight - 2050))) * 100);
    if (porgressLine < 100) { porgressBar.style.height = `${porgressLine}%`; }
  };
}
const mainNav = document.querySelector('.progress__line');
if (mainNav) {
  ...
  function windowScroll() {
    mainNav.classList.toggle('_active', mainNav.scrollTop > 30 || document.documentElement.scrollTop > 30);
    mainNav.classList.toggle('_raise', mainNav.scrollTop > 30 || document.documentElement.scrollTop > 30);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const radios = document.querySelectorAll('input[type="radio"][name="form-alcko"]');
  const textArea = document.querySelector('textarea[name="massage"]');

  // Добавляем каждой радио-кнопке обработчик события изменения
  radios.forEach((radio) => {
    radio.addEventListener('change', () => {
      // Проверяем, выбрана ли радио-кнопка "Другое"
      if (document.getElementById('radio-noalco').checked) {
        // Делаем текстовое поле активным
        textArea.disabled = false;
        textArea.focus(); // Опционально ставим фокус на текстовое поле
      } else {
        // Деактивируем текстовое поле и очищаем его
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function form_add_error(input) {
    ...
    // Создание и добавление нового сообщения об ошибке
    const errorText = 'Заполните поле'; // Текст ошибки
    const errorDiv = document.createElement('div');
    errorDiv.textContent = errorText;
    errorDiv.className = 'form__error'; // Установка класса для стилей ошибки
    input.parentElement.appendChild(errorDiv); // Добавление сообщения об ошибке в DOM
...
const date = new Date('2024-07-25T14:20:00');
function counts() {
  const now = new Date();
  gap = date - now;
  ...
  if (gap < 0) {
    document.querySelector('.in10-timer').classList.add('_active');
  }
  document.getElementById('d').innerText = `${days} `;
  document.getElementById('h').innerText = `${hours} `;
  document.getElementById('m').innerText = `${minutes} `;
  document.getElementById('s').innerText = seconds;
  document.getElementById('d').innerHTML = parseInt(days, 10);
  ...
  document.querySelector('.timer__d').textContent = declOfNum(days, ['день', 'дня', 'дней']);
  document.querySelector('.timer__h').textContent = declOfNum(hours, ['час', 'часа', 'часов']);
  document.querySelector('.timer__m').textContent = declOfNum(minutes, ['минута', 'минуты', 'минут']);
  document.querySelector('.timer__s').textContent = declOfNum(seconds, ['секунда', 'секунды', 'секунд']);
}
...
//Главный слайдер
let main_screen = new Swiper('.main-screen__scroll', {
  observer: true,
  observeParents: true,
  slidesPerView: 1,
  spaceBetween: 30,
  ...
  navigation: {
    nextEl: '.main-screen__arrow-next',
    prevEl: '.main-screen__arrow-prev',
  },
});
// Наши кейсы
let case_sliders = new Swiper('.case__sliders', {
  observer: true,
  observeParents: true,
  slidesPerView: 'auto',
  spaceBetween: 0,
...
//Карусель иконок
let case_carousel = new Swiper('.case__carousel', {
  observer: true,
  observeParents: true,
  slidesPerView: 'auto',
  spaceBetween: 0,
...
//Наш блог
let blog_content = new Swiper('.blog__content', {
  observer: true,
  observeParents: true,
  slidesPerView: 1,
  spaceBetween: 30,
...
//Страница с блогами
let All_blog = new Swiper('.page-blog__slider', {
  observer: true,
  observeParents: true,
  slidesPerView: 1,
  autoHeight: true,
